...
def makeQuiz1(mat0):                 # 4.5 문제집 풀이 리턴 - 수정1 (리스트 없이 간격 조절)
    for y1 in range(len(mat0)):
        print(f'{mat1[0][y1]:>4}', end='' if y1 < len(mat0) - 1 else '\n')
    for xh in range(1, len(mat0)-1):
        for yh in range(len(mat0[0])):
            print(f'{mat1[xh][yh]:^5}',end='' if yh < len(mat0)-1 else '\n')
...
def correct(listX):
    if len(listX) == 2:
        n1, n2 = listX
        if (n1.isalpha != True) and (n2.isalpha != True):
